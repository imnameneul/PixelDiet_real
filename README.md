...
- 지금 리마타에서 측정하는 주기가 어떻게 되는지, 최소 몇 분 단위로 알림을 보낼 수 있는 지 알아보고 시현용으로 수정할 수 있으면 수정(10초마다 알림 등)
 - 초기에 앱 목표시간을 지정할때 문제가 50% 알림과 70% 알림이 동시에 바로 오는 문제
 - 리마타가 70%, 100% 일때마다 경고 알림을 보내긴 하는데 알림바에만 조용히 알림을 보내는거 같아 난 무조건 알림이 오면 카톡처럼 화면 상단에 뜨게 하고 싶은데
 - 로그인할때만 백필 중인데(백필 안 하면 캘린더 화면 제대로 안뜸) 아예 복원 버튼 클릭하면 firestore에서 복원 가능하게 하기
